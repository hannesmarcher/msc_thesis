...
from src.lib.Label import uri_prefix, get_label
...
def violate_branch_conflict(branch_conflicts, p1_str, p2_str):
...
def violate_rules(FLAGS, p1_str, p2_str, c1_uri, c2_uri):
    violated = False
    if FLAGS.anchor_branch_conflict == 'yes':
        violated = violate_branch_conflict(branch_conflicts, p1_str=p1_str, p2_str=p2_str)
...
def negative_sampling(FLAGS, left_classes, right_classes, pos_mappings, left_paths, right_paths, left_names, right_names):
    neg_mappings = list()
...
def sample(FLAGS: argparse.Namespace):
    ...
    with open(FLAGS.anchor_mapping_file) as f:
        for i, line in enumerate(f.readlines()):
            ...
            if not n1 == '""' and not n2 == '""':
                if violate_rules(FLAGS, p1_str=n1, p2_str=n2, c1_uri=tmp2[0], c2_uri=tmp2[1]):
    ...
    if FLAGS.generate_negative_sample == 'yes':
        train_mappings = train_mappings * FLAGS.sample_duplicate + negative_sampling(FLAGS,
                                                                                     left_classes=left_classes,
                                                                                     right_classes=right_classes,
                                                                                     pos_mappings=train_mappings,
                                                                                     left_paths=left_paths,
                                                                                     right_paths=right_paths,
                                                                                     left_names=left_names,
                                                                                     right_names=right_names) + train_rv_mappings * FLAGS.sample_duplicate
        valid_mappings = valid_mappings * FLAGS.sample_duplicate + negative_sampling(FLAGS,
                                                                                     left_classes=left_classes,
                                                                                     right_classes=right_classes,
                                                                                     pos_mappings=valid_mappings,
                                                                                     left_paths=left_paths,
                                                                                     right_paths=right_paths,
                                                                                     left_names=left_names,
