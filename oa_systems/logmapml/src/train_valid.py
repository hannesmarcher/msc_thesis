...
from src.lib.Encoder import load_samples
from src.lib.Evaluator import threshold_searching
from src.lib.Network import nn_train, nn_predict
from src.lib.SiameseNetwork import siamese_nn_predict, siamese_nn_train
...
def valid(FLAGS, X1, X2, Y, valid_num):
    if FLAGS.nn_type.startswith('Siamese'):
...
def train_valid(FLAGS: argparse.Namespace) -> (str, str, float):
    start = datetime.now()
    ...
    for encoder_type in encoder_types:
        ...
        for nn_type in nn_types:
            ...
            print('\n------ %s START ------' % setting)
            train(FLAGS, X1=train_X1, X2=train_X2, Y=train_Y)
            threshold, f1, p, r, acc = valid(FLAGS, X1=valid_X1, X2=valid_X2, Y=valid_Y, valid_num = valid_num)
            print(
                '\n ##### best setting: %s, threshold: %.2f, precision: %.3f, recall: %.3f, f1: %.3f, acc: %.3f ##### \n' % (
                    setting, threshold, p, r, f1, acc))
            if f1 > best_f1_score:
                best_setting = (nn_type, encoder_type, threshold)
