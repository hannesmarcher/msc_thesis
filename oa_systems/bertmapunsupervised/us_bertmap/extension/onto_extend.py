...
class OntoExtend:
    ...
    def extend_mappings(self, max_iter: int = 1):
        ...
        while self.frontier and num_iter < max_iter:
            print(f"\tCurrent iteration: {num_iter}; Max number of iterations: {max_iter}")
            count = 0
            new_expansion = dict()
            for mapping in self.frontier.keys():
                src_iri, tgt_iri = mapping.split("\t")
                print(
                    f"[Time: {round(time.time() - start_time)}][Map {count} (out of {len(self.frontier.keys())})]: {src_iri} -> {tgt_iri}")
    ...
    def read_mappings_to_dict(
            mapping_file: Union[str, DataFrame], threshold: float = 0.0
    ) -> Dict:
        ...
        if type(mapping_file) is DataFrame:
            ...
            _df = pd.read_csv(mapping_file, sep="\t", na_values=Shared.na_vals, keep_default_na=False)
        mapping_dict = dict()
