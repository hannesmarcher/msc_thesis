...
class BERTTrainer:
    ...
    def train(self, train_args: TrainingArguments, resume_from_ckp: bool = False):
        ...
        Args:
            ...
            resume_from_ckp: whether resuming from previous checkpoint
        """
    ...
    def load_dataset(self, data: List, batch_size: int = 1024,
                     max_length: int = 512) -> Dataset:
        data_df = pd.DataFrame(data, columns=["sentence1", "sentence2", "labels"])
        dataset = Dataset.from_pandas(data_df)
        dataset = dataset.map(
            lambda examples: self.tokenizer(
                examples["sentence1"], examples["sentence2"], max_length=max_length, truncation=True
            ),
            batched=True,
