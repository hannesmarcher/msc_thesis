...
from us_bertmap.corpus.onto_corpus import OntoCorpus
...
class MergedOntoCorpus(OntoCorpus):
    def __init__(
        ...
        self.add_corpus = None
        self.add_corpus_info = None
        self.base_repr = None
        self.add_repr = None
    ...
    def config(self, base_onto_corpus: OntoCorpus, add_onto_corpus: OntoCorpus):
        ...
        self.corpus_info["nonsynonyms"]["removed_violations"] += self.add_corpus_info[
            "nonsynonyms"
        ]["removed_violations"]
    ...
    def train_val_split(
            self, val_ratio: float = 0.2, soft_neg_rate: int = 1, hard_neg_rate: int = 1
    ):
        ...
        soft_nonsynonyms = random.sample(
            semantic_pairs["soft_nonsynonyms"], soft_neg_rate * len(synonyms)
        )
        hard_nonsynonyms = random.sample(
            semantic_pairs["hard_nonsynonyms"], hard_neg_rate * len(synonyms)
        )
        # form the label data
        ...
        soft_nonsynonyms_for_ids = random.sample(
            semantic_pairs["soft_nonsynonyms"], soft_neg_rate * len(id_synonyms)
        )
        hard_nonsynonyms_for_ids = random.sample(
            semantic_pairs["hard_nonsynonyms"], hard_neg_rate * len(id_synonyms)
        )
        ...
        if val_ratio == 0.0:
            ...
            if len(label_data) == 0:
                train, val = [], []
                print("WARNING: see merged corpus")
            else:
                ##########################################
                # End Code Changes
                ##########################################
